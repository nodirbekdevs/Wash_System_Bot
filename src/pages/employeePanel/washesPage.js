...
const {wash_pagination} = require('./../../helpers/utils')
...
const ews2 = async (bot, chat_id, query_id, message_id, data, _id, kw, lang) => {
  let message, query
  ...
  if (kw === 'today') {
    ...
    query = {employee: employee.name, branch: employee.branch, status: 'washed'}
  }
  ...
  if ((data.split('#')[0] === 'left' || data.split('#')[0] === 'right') && data.split('#')[1] === 'wash') {
    ...
    await bot.editMessageText(report.text, {chat_id, message_id, parse_mode: 'HTML', kbb})
  }
...
module.exports = {employeeWashes, ews2}
